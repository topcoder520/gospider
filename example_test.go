...
func TestExample_1(t *testing.T) {
	...
	//spider.SetDownloader() //设置自己实现的下载器，不设置则使用默认下载器
	//spider.AddHandler() //添加处理html文本的处理器，不设置使用默认的
	//spider.AddPipeline() //添加结果处理器，负责处理AddHandler()返回的数据，不设置使用默认的
	//spider.SetScheduler() //设置自己实现的调度器，不设置使用默认的
	spider.Run()
}
